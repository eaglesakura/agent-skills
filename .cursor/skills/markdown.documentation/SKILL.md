...
* ドキュメントは `DO` `推奨` を示し、DO NOTはexampleやナレッジベースのみに記載する
...
1. **概要**: ドキュメントの冒頭に配置する
2. **ナレッジベース**: ドキュメントの末尾付近に配置する（参考リンクの直前）
...
すべてのドキュメントには、「ナレッジベース」セクションを配置する（必須）。参考リンクがある場合は、その直前に配置する。
...
* セクション見出しは「`ナレッジベース`」とする
* 各項目の見出しは `DO:` または `DO NOT:` で始める
* 見出し本文は1行で内容を端的に表す
* 二重否定による混乱を防ぐため、テキストは `DO` / `DO NOT` で揃える
* 各項目は重要度の高い順に並べる

### DO

* `DO:` 見出しに、推奨する実装・運用を記載する
* 見出しは肯定形で記述する（例: `DO: Dependency Injectionを使用する`）
* 必要に応じて、推奨パターンの説明やコード例を付与する

### DO NOT

* `DO NOT:` 見出しに、避けるべき実装・運用を記載する
* 見出しは二重否定にならないよう記述する（例: `DO NOT: static変数を使用する`）
* 避けるべき理由を箇条書きで記載する
* 非推奨パターンのコード例と、推奨される書き換えパターンのコード例を併記する
...
2. **ドキュメント構造の確認**
   * 必須セクション（概要、ナレッジベース）が含まれていることを確認する
