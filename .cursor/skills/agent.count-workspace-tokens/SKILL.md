---
name: agent.count-workspace-tokens
description: >-
...
metadata:
  author: "@eaglesakura"
---
# AI Agent / Count Workspace Tokens
...
5. 結果を残すなら `.ai-agent/tmp/` へ（`agent.temporary`）
...
SCRIPT=".cursor/skills/agent.count-workspace-tokens/scripts/count_workspace_tokens.py"
python3 "$SCRIPT" --root .
# multi-root は --root を繰り返し。ユーザー SKILL 込みは --include-user-skills。JSON は --json。
```

パス解決は `resolve-file-path` に従う。トークナイザは `tiktoken` の `o200k_base`、無ければ `chars/4`。
