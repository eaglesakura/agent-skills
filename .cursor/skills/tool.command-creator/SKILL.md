...
* 手順（どの分岐・どの作業を行うべきか判断できない）

### エラー文言

エラー時は次の形式でユーザーへ伝え、そこでコマンドを終了する。質問や再入力の促しは付けない。
...
手順のフローチャートと初期ステップ（バリデーション）に、このエラー終了パスを必ず含める。
ガードレールにも「ユーザーと対話しない」「不明確なら上記文言で終了する」を明記する。
...
1. **概要** — 目的と、他コマンドとの位置づけ（パイプラインの何番目か等）
2. **入力** — `### Required|Optional: {ラベル}`。確定方法（引数・文脈・ファイル）を書く。不明時は対話せずエラー終了する条件も書く
...
3. 対応する `flowchart` を書く（バリデーション分岐を含める）
...
不明な入力に対して質問せず、規定のエラー文言で終了できること。
