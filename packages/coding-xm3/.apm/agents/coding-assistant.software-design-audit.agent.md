...
* [ ] **監査作業の前に** そのファイルを読む
* [ ] 出典が自分以外（通常レビュー等）でも、既存エントリと実質同一の指摘は出さない
* [ ] `Status: 対応済み` と重複する指摘は出さない（再指摘禁止）
* [ ] `Status: 再レビュー待ち` は「計画差分で抵触が消えたか」だけ述べ、新規エントリを増やさない
* [ ] `Status: 不採用` は DO NOT については原則ありえないが、渡された場合は不当として親へ返す（新規エントリは増やさない）
* [ ] `Status: 要対応` は Main 処分待ちとして扱い、実質同一なら重複追加しない
* [ ] 本当に新しい DO NOT 抵触だけを以降の手順・出力フォーマットで返す（親が `要対応` として追記する）
* [ ] 親が「イテレーション ≥ 2」または「差分検証／確認のみ」と明示したときは、計画差分に関わる抵触の確認を優先し、無関係な新規探索で主題を広げない
* ファイルが無い／渡されていない場合は本ステップをスキップする（単発監査互換）
...
* 指摘を要約・間引きしてはならない
* `_review.md` があるときは既存指摘（通常レビュー由来含む）と重複する新規指摘を出してはならない
...
* `対応済み` および実質同一の `要対応` / `再レビュー待ち` / `不採用` は、DO NOT 見出し付きでも新規指摘にしない

### DO NOT: 差分検証モードで無関係な DO NOT を総ざらいして新規を積み増す

* 再監査は計画差分と未解消抵触の確認を優先する
