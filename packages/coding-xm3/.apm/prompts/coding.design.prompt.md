...
* Sub Agentによるレビュー実施
  * `/coding-assistant.software-design-reviewer` Sub Agent を実行してレビューを受ける
    * セキュリティおよびコーディングの一般的アンチパターンに抵触していないか（cascade とは別枠）
    * 設計・実装観点の基準は次の **cascade** で選ぶ。上から「使える指示があるか」を判定し、**最初にヒットした 1 段だけ**を使う。下位へは降りない。1〜6 を同時に総ざらいしない
      1. プロンプトに特別な指示がある → その指示を優先してレビューする
      2. （1 が無い）プロジェクト内ドキュメントに適用可能な指示がある → docs の指示を優先してレビューする
      3. （2 が無い）既存・関連コードの慣習が特定できる → その慣習をもとにレビューする
      4. （3 が無い）公式推奨パターンが特定できる → 公式推奨をもとにレビューする
      5. （4 が無い）コミュニティ推奨が特定できる → コミュニティ推奨をもとにレビューする
      6. （5 が無い）一般的なコーディングパターンでレビューする
    * Sub Agent プロンプトには選んだ cascade 段を明示する（例: `今回の基準: プロジェクト docs`）
...
### DO NOT: 通常レビューで cascade 全段を同時に総ざらいする

* 理由: 上位段が使えるなら下位は使わない。全視点レビューはコストとノイズになるためである
