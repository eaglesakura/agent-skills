...
本 SKILL は次を **コマンド経路経由** で使う（Sub Agent を本 SKILL から直接並べない。`/coding.design` / `/coding.execute` に任せる）。
...
DO NOT 残が 1 以上のあいだ、**レビューのみ** を繰り返す。Sub Agent を本 SKILL から直接起動せず、コマンドに任せる。
...
DO NOT 残の解消は `/coding.design … レビューのみ` に任せ、本 SKILL から Sub Agent を直叩きしない。
