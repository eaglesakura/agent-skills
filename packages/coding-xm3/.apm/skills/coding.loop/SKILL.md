...
description: >-
  ...
  詳細設計ループが指摘を親へ渡した場合は本 SKILL が採用（再レビュー待ち→Review 確認）／不採用（対応済み＋記録）を決め、ゲート完走する。
...
詳細設計ループが指摘の一部を親へ渡した場合は、本 SKILL が上位として対応判断を行う。採用は `再レビュー待ち` → Review 確認で `対応済み`、不採用は Main が直接 `対応済み`（本文に「提案を不採用とした」＋理由）とし、全指摘が `対応済み` になるまで execute へ進まない。
通常レビューと DO NOT 監査は同一の `{stem}_review.md` を先行閲覧し横断重複しない。通常レビュー観点は cascade（最初にヒットした 1 段だけ。全視点総ざらい禁止）。
Status 凡例は `references/review-template.md` 正本（`要対応` / `再レビュー待ち` / `対応済み`）。確認レビューは必要 Agent のみ。同一 `{stem}_{N}` のレビュー起動は最大 3 回（内側の発散防止）。
...
* Status は `references/review-template.md` の凡例に従う（`要対応` / `再レビュー待ち` / `対応済み`）
...
flowchart TD
    ...
    HasTodo -->|はい| Disposition["Main処分<br/>採用=再レビュー待ち<br/>不採用=対応済み+記録"]
    Disposition --> Gate
    HasTodo -->|いいえ| Cap{"当該Nのレビュー起動<br/>回数 less than 3？"}
    Cap -->|いいえ| NoExec["executeせず残課題メモ<br/>外側の完了/タイムアウト判定へ"]
    NoExec --> DoneQ
    Cap -->|はい| FixReview["必要Agentで確認レビュー<br/>差分検証のみ"]
    FixReview --> Gate
...
2. `要対応` は Main が処分する。DO NOT は必ず採用（`再レビュー待ち`）。その他は採用（`再レビュー待ち`）または不採用（`対応済み`＋「提案を不採用とした」記録）
3. `再レビュー待ち` は Review Agent の確認で `対応済み`（または未解消なら `要対応` へ戻す）。不採用は Main 処分時点で `対応済み` となり確認不要。確認は必要 Agent のみ。同一 `{stem}_{N}` のレビュー起動は最大 3 回
...
既定の詳細設計ループはレビューと 1 回分の改善まで含む。計画へ反映した指摘は `再レビュー待ち` になる。終了時点で未完了 Status（`要対応` / `再レビュー待ち`）が残っていればステップ4へ進む。すべて `対応済み` ならステップ4をスキップしてステップ5へ進んでよい。
...
| その他 | **不採用** | Status を `対応済み` にし、同エントリ本文に「提案を不採用とした: {理由}」を書く（Review 確認なし） |
...
* 採用経路では Main 単独で `対応済み` にしない（Review 確認が必要）。不採用処分時のみ Main が直接 `対応済み` にしてよい

同一指摘が再び `要対応` に戻ってきた場合は、既存の不採用理由がなお妥当なら再度 `対応済み`（「提案を不採用とした」＋理由）にしてよい。妥当でなければ採用して `再レビュー待ち` にする。

処分後の記録:

* `pending_confirm`: `再レビュー待ち` が1件以上あるか
...
1. `pending_confirm` が true（`再レビュー待ち` がある）
2. 当該 `{stem}_{N}` のレビュー起動回数 < 3

満たさない場合:

* すべて `対応済み` → ゲート通過（ステップ5へ）
* レビュー起動回数が既に 3 で未完了が残る → execute せず残課題をメモし、外側の完了／タイムアウト判定へ戻る（採用経路の `再レビュー待ち` を Main 単独で `対応済み` にしない）
...
```text
while 未完了 Status（要対応 / 再レビュー待ち）が1件以上:
  if 要対応 がある:
    4.2 で全件処分する
    continue
  if レビュー起動回数 >= 3:
...
* `_review.md` に未完了 Status（`要対応` / `再レビュー待ち`）があるまま execute しない
* DO NOT 抵触を不採用で逃げない（必ず採用して `再レビュー待ち` → Review 確認で `対応済み`）
...
`要対応` / `再レビュー待ち` を残したまま execute しない。未決解消の合議とレビュー処分を混同しない。

### DO: 確認レビューは必要 Agent だけで、差分検証に絞る

採用後は `再レビュー待ち` → Review 確認で `対応済み`。却下は Main が直接 `対応済み`（「提案を不採用とした」＋理由）。同一 N で最大 3 回。軽微指摘の再発火で内側を伸ばさない。
...
### DO NOT: 処分のたびに初回フルレビューを繰り返す／採用経路で Main 単独で対応済みにする

不要 Agent の再起動、および確認レビューでの `中`/`低` 新規の追記は、内側ループの発散とコスト増になる。採用経路の `対応済み` は双方承認のみ。不採用は Main が理由付きで直接 `対応済み` にしてよい。
