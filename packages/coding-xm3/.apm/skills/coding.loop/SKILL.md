...
name: coding.loop
description: >-
  要件定義済み計画に対し、詳細設計→レビュー指摘の処分完了（DO NOT 残ゼロ＋保留なし）→実装（各ステップ後は必ず /coding.comment）を完了条件達成まで自律ループする SKILL。
  `/coding.design` が指摘を保留した場合は本 SKILL が承認／否認を再判断し、再レビューまで完走する。
...
本 SKILL 実行中は、単発 `/coding.design` 末尾の「ユーザー承認待ち」で止まらず、ゲート（レビュー処分完了・DO NOT 残ゼロ・完了条件・タイムアウト）に従って execute / 次ループへ進む。
`/coding.design` 単体は指摘の一部を保留しうる。その場合は本 SKILL が上位として対応判断（承認／否認）を再実行し、未処理のまま execute へ進まない。
...
flowchart TD
    ...
    Design --> Disposition["レビュー結果の処分<br/>DO NOT必須修正 / 他は承認or否認"]
    Disposition --> Gate{"DO NOT残=0<br/>かつ未処理・保留=0 か"}
    Gate -->|いいえ| FixReview["反映または否認ログ更新<br/>→ /coding.design 再レビュー"]
    FixReview --> Disposition
...
2. レビュー結果を処分する。DO NOT は必ず修正、その他は全部／一部承認または否認。`/coding.design` の保留は本 SKILL が再判断する
3. DO NOT 残 0 かつ未処理・保留 0 になるまで修正／否認ログ更新と再レビューを繰り返す（大幅な設計作り直しは外側で `{stem}_{N+1}` を切る）
...
既定の `/coding.design` はレビューと 1 回分の改善まで含む。終了時点で DO NOT 残・未処理指摘・保留が残っていればステップ4へ進む。ゲートを既に満たすならステップ4をスキップしてステップ5へ進んでよい。

### ステップ4: 内側ループ（レビュー結果の処分 → 再レビュー）

単発 `/coding.design` は指摘の一部を「保留」にしうる。本 SKILL はその上位として、**保留・未処理を残したまま execute しない**。通常のレビュー Sub Agent は `/coding.design` に任せ、本 SKILL から直起動しない（合議例外は未決解消の第3段のみ）。

#### 4.1 指摘の収集

直近の `/coding.design` 出力（レビュー指摘内容・DO NOT 監査結果）と分割計画から、未処理の指摘を列挙する。

* DO NOT 監査の抵触
* reviewer 指摘のうち、まだ計画へ反映されておらず、否認ログにも無いもの
* design 側が明示的に保留した指摘
...
| 種別 | 判断 | 対応 |
| --- | --- | --- |
| DO NOT | **否認不可**。必ず承認 | 計画を修正し、抵触が消えるまで改善する |
| その他 | **全部または一部を承認** | 承認した範囲を計画へ反映する |
| その他 | **否認** | 否認内容と否認理由を分割計画へ明記する（下記ログ） |

* 一部承認した場合、残った部分も同じ指摘単位で承認か否認かを決め、未処理のまま残さない
* 「保留のまま次へ」は本 SKILL では禁止。design が保留した指摘はここで必ず再判断する
* 未決の実装詳細が処分に必要なら、先に「未決の追加要件・実装詳細の解決優先度」で埋めてから処分する

否認は分割計画に次の形式で残す（レビュアーが再レビューで参照できるようにする）。

```markdown
## レビュー否認ログ

### 否認: {指摘のサマリ}
* 対象レビュー: {reviewer / 観点の識別}
* 否認する内容: {何を採らないか}
* 否認理由: {なぜ採らないか。既存コード・ドキュメントへの参照があればパス}
* 記録時点の外側ループ: N={N}
```

同一指摘が再レビューで戻ってきた場合は、既存否認理由がなお妥当なら理由を再確認して維持してよい。妥当でなければ承認に切り替えて反映する。

#### 4.3 再レビューとゲート

処分（反映または否認ログ更新）のあと、必ず再レビューする。

```text
do {
  4.1 で未処理・保留・DO NOT 残を確認する
  あれば 4.2 で全件処分する
  /coding.design {stem}_{N}.md 実装修正を行い、再レビュー
    // 計画差分が無く否認ログのみの更新なら「レビューのみ」でもよい
} while (DO NOT 残 >= 1 || 未処理指摘 >= 1 || 保留指摘 >= 1)
```

内側ゲート（すべて満たしてからステップ5へ）:

1. DO NOT 監査の残件数 = 0
2. 保留・未処理の指摘 = 0（全指摘が承認反映済み、または否認ログに理由付きで記録済み）

* 内側の収束より外側のタイムアウト判定が優先される
* 「レビューのみ」は詳細設計の書き直し（design のステップ1）をスキップする。大幅な作り直しが必要なら外側で `N` を進め新しい分割計画を切る
* 否認ログだけでゲートを「通過したつもり」にしない。再レビュー後に新規指摘が出たら、それらも 4.2 で処分する
...
* DO NOT 残、または保留・未処理のレビュー指摘があるまま execute しない
* DO NOT 抵触を否認で逃げない（必ず修正して残 0 にする）
* `/coding.design` の保留を「ユーザー待ち」や「後で」にせず、本 SKILL で承認／否認を再判断する
...
### DO: 内側ゲートは「レビュー処分完了」である

DO NOT 残ゼロに加え、全指摘が承認反映か否認ログ済みであること。design の保留は上位の本 SKILL が拾う。未決解消の合議とレビュー処分を混同しない。
...
### DO NOT: 単発 design の「ユーザー承認待ち」や「保留」で外側を止める

本 SKILL のゲートはレビュー処分完了・DO NOT 残ゼロ・完了条件・タイムアウト・ユーザー明示停止である。毎回の人間承認や design 側の保留放置を外側の待機条件にしない。
