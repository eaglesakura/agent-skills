---
source: https://mas.owasp.org/MASTG/tests/ios/MASVS-PLATFORM/MASTG-TEST-0376/
scopes:
  - test
  - ios
  - mobile
...
* 本ドキュメントは OWASP MASTG Test「References to Native Bridge APIs in WebViews」（iOS / プラットフォーム連携）を、テストナレッジ／DO NOT 監査向けに蒸留したものである。
* 公式ステータスは current である。検証手順の正本は公式記事とする。
* 要旨: iOS apps can establish a bidirectional communication channel between JavaScript and native code through WebView-native bridges. When using WKWebView, message handlers are registered on the WKUserContentController via add(_:name:)) (or addScriptMessageHandler:name: in Objective-C). This defines a WKScriptMessageHandler that handles messages sent from JavaScript using window.webkit.messageHandlers..postMessage(...).
* メタ: type: static, code; profiles: L1, L2; weakness: MASWE-0034; knowledge: MASTG-KNOW-0076
* 正本: <https://mas.owasp.org/MASTG/tests/ios/MASVS-PLATFORM/MASTG-TEST-0376/>
* 関連制御群: `MASVS-PLATFORM`
* 関連 Knowledge 索引: `docs/security/mas.owasp.org/knowledge/0000-index.md`

## References to Native Bridge APIs in WebViewsのテスト実施

テスト目的・手順・合否基準を固定し、実装レビューとセキュリティテストで再利用する。

### References to Native Bridge APIs in WebViewsのテスト実施の補足
...
### References to Native Bridge APIs in WebViewsのテスト実施の実装例
...
* 観測期待: The output should contain a list of locations in the binary where WKUserContentController.add(_:name:) is called.
```
...
# DO
- MASTG-TEST-0376 の合否を Evaluation に照らして記録する
- 失敗時は関連 MASWE / 修正方針 / 再テスト条件を起票する
```

## 参考リンク

* 本 Test: <https://mas.owasp.org/MASTG/tests/ios/MASVS-PLATFORM/MASTG-TEST-0376/>
